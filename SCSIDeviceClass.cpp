...
int SCSIDeviceClass::processDeviceIdentification(SCSI_CBD_INQUIRY  &cbd, uint32_t len) {
	...
	ID1.protocol_identifier = 0x3; // SBP2
	ID1.association = 0;
...
int SCSIDeviceClass::processInquiryUnitSerialNumberPage(SCSI_CBD_INQUIRY  &cbd, uint32_t len) {
	...
	unitSerialNumberPage.page_length = 12;
	uint8_t product_serial_number[8] = {'1','2','3','4','5','6','7','8'};
	memcpy(unitSerialNumberPage.product_serial_number, product_serial_number, 8);
	memcpy(transferData, &unitSerialNumberPage, 12);
	transferLength = 12;
	return 12;
}
